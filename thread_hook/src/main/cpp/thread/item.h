...
namespace threadhook {
    ...
    public:
        ...
        uintptr_t pc[threadhook::Constant::kMaxCallStackDepth]{};
        std::string native_stack;

        ThreadCreateArg() {}

        ~ThreadCreateArg() { memset(pc, 0, sizeof(pc)); native_stack.clear();}
    };
    ...
    public:
        ...
        HookAddInfo(int tid, long long time, pthread_t pthread, bool isThreadDetached,
                    ThreadCreateArg *thread_create_arg, char *threadName) {
            this->tid = tid;
            this->time = time;
            this->pthread = pthread;
            this->is_thread_detached = isThreadDetached;
            this->create_arg = thread_create_arg;
            this->thread_name.assign(threadName);
        };
    };
